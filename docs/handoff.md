...
### ② 本文が入るので、閾値の見直しが要る（次にやること）

2026-08-15 に一次情報（文化庁・文部科学省）を有効にした。**次の収集から本文が入る。**
1記事あたりの共起単位が「見出し1文」から「本文の数十文」に変わるので、
`min_df: 4` / `min_cooc: 3` / `min_npmi: 0.25` は見出しだけの390件に合わせた値のまま使えない。

やること: 収集後の `out/report.md` と `out/articles.html` を見て、
(a) 官公庁ページの定型句（お問い合わせ先・PDF・担当課…）を `dict/stopwords.txt` に足す、
(b) 閾値を調整する。**閾値より先に (a)** を見ること（定型句が上位に来るのは閾値の問題ではない）。

これが片付くと「文化資源が保存の文脈で語られたのか観光の文脈なのか」が初めて見える
